...
WINDOWS_SPEC=(
  "14:00-17:00|1786341600|1786352400|10800"
  "17:00-20:00|1786352400|1786363200|10800"
  "18:00-19:00|1786356000|1786359600|3600"
  "19:00-20:00|1786359600|1786363200|3600"
)
...
printf '%-12s %10s %10s %9s   %10s %10s %9s\n' \
  "system" "14-17 (3h)" "17-20 (3h)" "SERIES" "18:00-19" "19:00-20" "CONTROL"
for entry in "${SYSTEMS[@]}"; do
  IFS='|' read -r sys base auth <<< "${entry}"
  g() { local k="$1" x; for x in "${S[@]}"; do [[ "${x}" == "${k}|${sys}|"* ]] && { echo "${x##*|}"; return; }; done; echo err; }
  a="$(g '14:00-17:00')"; b="$(g '17:00-20:00')"; c="$(g '18:00-19:00')"; d2="$(g '19:00-20:00')"
  rt() { python3 -c '
import sys
x,y=sys.argv[1:3]
print(f"{int(x)/int(y):.2f}x" if x.isdigit() and y.isdigit() and int(y) else "-")' "$1" "$2"; }
  printf '%-12s %10s %10s %9s   %10s %10s %9s\n' \
    "${sys}" "${a}" "${b}" "$(rt "${a}" "${b}")" "${c}" "${d2}" "$(rt "${c}" "${d2}")"
done
